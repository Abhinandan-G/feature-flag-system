...
const adminSignup = async (req, res) => {
  try {
    const { email, username, password, org_id } = req.body;
    if (!email || !username || !password || !org_id) {
      return sendError(res, 'All fields are required', 400);
    }
    const result = await authService.adminSignup({ email, username, password, org_id });
...
const adminLogin = async (req, res) => {
  try {
    ...
    const result = await authService.adminLogin({ email, password, org_id });
    return sendSuccess(res, result, 'Login successful');
  } catch (err) {
    return sendError(res, err.message, err.status || 500);
  }
